...
const cryptoRoot = process.env.FABRIC_CRYPTO_ROOT || path.resolve(
    __dirname, '../../fabric-network-config/crypto-config'
);
...
async function credentialRequestReferences() {
    const result = await skillNetwork.getContract('skill-manager')
...
app.post('/v1/transactions', async (request, response) => {
    try {
        ...
        const contract = networks.get(command.contract).getContract(command.contract);
