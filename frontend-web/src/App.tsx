...
export function App() {
  ...
  async function signOutBusiness() {
    ...
    setBusinessActor(null);
    disconnect();
    setError('Business profile signed out. Switch accounts in MetaMask, then reconnect.');
  }
    ...
    <Box className="app-shell">
      <AppBar position="sticky" color="transparent" elevation={0} className="topbar">
        <Toolbar>
          ...
          <Chip color="success" variant="outlined" size="small" label="Fabric · SynapseNet" />
          {connected && <Button sx={{ ml: 1 }} size="small" variant="text"
            onClick={() => setBusinessAction('identity')}>
            {businessActor?.displayName || businessActor?.actorId || 'Business sign in'}
          </Button>}
          <Button sx={{ ml: 2 }} variant={connected ? 'outlined' : 'contained'} onClick={connected ? disconnect : connect} startIcon={<AccountBalanceWalletOutlined />}>
            {address ? shortAddress(address) : 'Connect MetaMask'}
          </Button>
        </Toolbar>
      </AppBar>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 3, md: 5 } }}>
        ...
        <Grid container spacing={4} alignItems="end" className="hero">
          <Grid size={{ xs: 12, md: 8 }}>
            ...
            <Typography variant="h1" sx={{ mt: 2, fontSize: { xs: '2.5rem', md: '4rem' } }}>Your verified career,<br /><span>under your control.</span></Typography>
            <Typography color="text.secondary" sx={{ mt: 2, maxWidth: 650, fontSize: '1.08rem' }}>
              Own verified skills, roles, and certificates. Authorize every action from your wallet and disclose only what you choose.
            </Typography>
          </Grid>
          <Grid size={{ xs: 12, md: 4 }}>
            <Card className="identity-card"><CardContent>
              <Typography variant="overline" color="secondary">Identity status</Typography>
              <Typography variant="h5" sx={{ mt: 1 }}>{connected ? 'Wallet verified' : 'Not connected'}</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ my: 2 }}>
                {address ?? 'Connect MetaMask to sign a one-time challenge. SynapseNet never receives your private key.'}
              </Typography>
              <LinearProgress variant={connected ? 'determinate' : 'indeterminate'} value={100} color={connected ? 'secondary' : 'primary'} />
            </CardContent></Card>
          </Grid>
        </Grid>
        ...
        <Grid container spacing={3}>
          <Grid size={{ xs: 12, md: 7 }}>
            <Card className="wallet-card credential-card">
                <CardContent>
                  <Stack direction="row" justifyContent="space-between"><Avatar><CheckCircleRounded /></Avatar><Chip label="Credential wallet" /></Stack>
          ...
          <Grid size={{ xs: 12, md: 5 }}>
            <Card className="wallet-card token-card">
              <CardActionArea onClick={() => setDialog('tokens')}>
                <CardContent>
                  <Stack direction="row" justifyContent="space-between"><Avatar><BoltRounded /></Avatar><Chip color="secondary" label="SNT utility" /></Stack>
      ...
      <Dialog open={dialog !== null} onClose={() => setDialog(null)} fullWidth maxWidth={dialog === 'credentials' ? 'xl' : 'md'}>
        <DialogTitle>{dialog === 'credentials' ? `${transactionStatus ? `${transactionStatus.replace('_', ' ')} ` : ''}Credential transactions` : 'Token transactions'}</DialogTitle>
        <DialogContent>
      ...
      <Dialog open={diagnosticsOpen} onClose={() => setDiagnosticsOpen(false)} fullWidth maxWidth="md">
        <DialogTitle>MetaMask connection diagnostics</DialogTitle>
        <DialogContent>
