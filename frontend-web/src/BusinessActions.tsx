import {
  ...
  DialogContent, DialogTitle, Divider, FormControlLabel, MenuItem, Stack, Tab, Tabs,
...
export function BusinessActions({
  ...
  async function authenticate(form: FormData) {
    ...
    try {
      ...
      setNotice({ severity: 'success', text: `Signed in as ${identity.displayName || identity.actorId}.` });
    } catch (reason) {
      setNotice({ severity: 'error', text: messageOf(reason) });
    } finally {
      setBusy(false);
  ...
  async function register(form: FormData) {
    ...
    try {
      ...
      await onAuthenticated(identity);
      setNotice({ severity: 'success', text: 'Professional profile created and linked to your wallet.' });
    } catch (reason) {
      setNotice({ severity: 'error', text: messageOf(reason) });
    } finally {
      setBusy(false);
    ...
    <DialogContent>
      ...
      {actor && action === 'identity' && <Stack spacing={2} sx={{ mt: 1 }}>
        <Alert severity="success">
          Signed in as {actor.displayName || actor.actorId} ({actor.role}).
        </Alert>
        <Alert severity="info">
          A MetaMask address can represent only one SynapseNet actor. Sign out here,
          switch MetaMask accounts, and reconnect to use another role.
        </Alert>
