...
POLICY_FILE="${1:-config/trust-policy.dev.json}"
...
if cli peer chaincode query -C "${CHANNEL_NAME}" -n "${TRUST_NAME}" \
    -c '{"Args":["getActivePolicy"]}' >/dev/null 2>&1; then
    FUNCTION="activatePolicy"
fi

cli peer chaincode invoke \
    -o orderer.synapsenet.com:7050 --tls --cafile "${ORDERER_CA_PATH}" \
    -C "${CHANNEL_NAME}" -n "${TRUST_NAME}" \
    -c "$(jq -cn --arg function "${FUNCTION}" --arg policy "${POLICY}" \
