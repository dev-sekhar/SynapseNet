...
invoke "{\"function\":\"registerUser\",\"Args\":[\"${USER_ID}\",\"Smoke Test User\"]}"
invoke "{\"function\":\"registerEnterprise\",\"Args\":[\"${ENTERPRISE_ID}\",\"Smoke Test Enterprise\",\"${REVIEWER_ID}\"]}"
...
invoke "{\"function\":\"submitCredentialRequest\",\"Args\":[$(printf '%s' "${PAYLOAD}" | jq -Rs .)]}"
invoke "{\"function\":\"reviewCredentialRequest\",\"Args\":[\"${REQUEST_ID}\",\"${REVIEWER_ID}\",\"approve\",\"Smoke test evidence verified\"]}"

RESULT="$(cli peer chaincode query -C "${CHANNEL_NAME}" -n "${CHAINCODE_NAME}" \
    -c "{\"function\":\"getWallet\",\"Args\":[\"${USER_ID}\"]}")"
grep -q "${TITLE}" <<<"${RESULT}"
